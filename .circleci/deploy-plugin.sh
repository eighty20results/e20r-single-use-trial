...
TAG_INFO=$(git rev-list --tags --max-count=1)
LATEST_GIT_TAG=$(git describe --tags "${TAG_INFO}")
# Remove the "v" at the beginning of the git tag
LATEST_SVN_TAG=${LATEST_GIT_TAG:1}

# Check if the latest SVN tag exists already
svn ls "https://plugins.svn.wordpress.org/$WP_ORG_PLUGIN_NAME/tags/$LATEST_SVN_TAG"
error=$?
if [ $error == 0 ]; then
